...
class Scraper():
    ...
    def __init__(self, api=None):
        ...
        print(f'json of images meta data will be saved here ==>> {data_dir}')
        failed = 'failed.txt'
    ...
    def image_url(self, page):
        ...
        response = requests.get(
            'http://api.scraperapi.com', params=payload).text
        ...
        metrics_dict = {i.split(':')[0]: i.split(':')[1]
                        for i in metrics if '' != i}
    ...
    def get_image(self, image_dict):
        ...
        payload = {'api_key': self.scraperapi, 'url': url}
        response = requests.get(
            'http://api.scraperapi.com', stream=True, params=payload)
    ...
    def get_new(self):
        ...
        print(
            f'succesfully scraped {len(self.current)} : to rescrape {len(re_scrape)}')

        self.all_challenges = np.array(self.get_challenges())
        self.challs_dict = {[split_url.split('&amp;') for split_url in challange_url.split(
            '=')][1][0]: challange_url for challange_url in self.all_challenges}
        ...
        self.new_challenges = np.setdiff1d(
            self.all_id_challenges, self.current)
    ...
    def parse_html(self):
        links = [
            ...
            self.soup.find_all('td', align="center",
        ...
        comp_data['description'] = description
            ...
            while len(self.new_challenges) == test:
                try:
                    with open(self.fid + '.json', 'r') as fid:
                        ...
                        self.new_challenges = np.setdiff1d(
                            self.challs_touched, self.all_challenges)
    ...
    def all_to_dict(self, **kwargs):
        if kwargs['success']:
            ...
            self.data_dict[chall_id]['status'] = {
                'scrape_items_match':
                len(self.meta['url']) == len(
                    self.meta['n_votes']) == len(self.meta['meta_data']),
